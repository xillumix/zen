...
class sc_rawcert(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        mark_logs("\nNode 1 creates the SC spending " + str(sc_amount) + " coins ...", self.nodes, DEBUG_MODE)
        sc_address = "fade"
        ...
        mark_logs("\nNode0 generating 5 block", self.nodes, DEBUG_MODE)
        self.nodes[0].generate(5)
        self.sync_all()

        mark_logs("\nNode0 generating 1 block", self.nodes, DEBUG_MODE)
        epn = 1
        eph = self.nodes[0].generate(1)[0]
        self.sync_all()
        ...
        dump_sc_info(self.nodes, 2, scid, DEBUG_MODE)
